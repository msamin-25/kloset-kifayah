...
import { Search, User, LogOut, Bell, MessageSquare, PlusSquare } from 'lucide-react';
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-beige/95 backdrop-blur-sm border-b border-rose-light h-20">
          ...
          {currentUser ? (
            <div className="flex items-center space-x-4">
              ...
              <div 
                ...
                onClick={() => onNavigate('me')}
              >
                <img src={currentUser.avatarUrl} alt={currentUser.displayName} className="w-full h-full object-cover grayscale hover:grayscale-0" />
              </div>
            </div>
          ) : (
            <button 
      ...
      <div className="lg:hidden flex items-center justify-between h-full px-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div 
            ...
            onClick={() => onNavigate('me')}
          >
            <img src={currentUser?.avatarUrl} alt="Avatar" className="w-full h-full object-cover grayscale" />
          </div>
        </div>
      </div>
      
      {/* Editorial Banner - thin rose line */}
      ...
      <div className="absolute -bottom-6 left-0 right-0 flex justify-center pointer-events-none">
        <div className="bg-espresso text-white px-4 py-1 text-[8px] uppercase tracking-[0.3em] font-medium border border-rose shadow-sm">
          Community Wardrobe • Conscious Choice
        </div>
      </div>
    </header>
  );
