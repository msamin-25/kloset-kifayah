...
const Home: React.FC<HomeProps> = ({ onAuth }) => {
  return (
    <div className="min-h-screen w-full bg-beige flex flex-col relative overflow-hidden">
      {/* Decorative background touches - more pink focus */}
      ...
      <div className="flex-grow flex flex-col lg:flex-row items-center justify-center max-w-[1400px] mx-auto px-6 lg:px-12 py-10 lg:py-0 gap-10 lg:gap-20 w-full">
        ...
        <div className="w-full lg:w-1/2 relative group max-w-md lg:max-w-none">
          ...
          {/* Floating Label - Rose themed */}
          <div className="absolute -bottom-4 -right-4 lg:-bottom-8 lg:-right-8 bg-rose text-white px-6 py-3 lg:px-8 lg:py-4 z-20 shadow-xl border border-white/20">
             <span className="text-[10px] lg:text-[11px] uppercase tracking-[0.4em] font-bold">Est. 2024</span>
          </div>
          {/* Ribbon Accents */}
        ...
        <div className="w-full lg:w-1/2 flex flex-col items-center lg:items-start text-center lg:text-left space-y-8 lg:space-y-12">
          <div className="space-y-2 lg:space-y-4">
            ...
            <p className="text-xs md:text-sm lg:text-base text-espresso/70 max-w-md leading-relaxed tracking-wide font-light lg:pt-8">
              A curated sanctuary for the conscious wardrobe. Discover, borrow, and share high-fashion modest wear with a global community of style enthusiasts.
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 md:gap-8 pt-8 lg:pt-10 border-t border-hairline w-full">
            <div className="space-y-1 lg:space-y-2">
              <span className="block text-[8px] uppercase tracking-[0.2em] font-bold text-rose">Circular</span>
              <span className="block text-[9px] lg:text-[10px] uppercase tracking-widest font-bold">Rent • Buy</span>
            </div>
            <div className="space-y-1 lg:space-y-2">
              <span className="block text-[8px] uppercase tracking-[0.2em] font-bold text-rose">Ethical</span>
              <span className="block text-[9px] lg:text-[10px] uppercase tracking-widest font-bold">Curated</span>
            </div>
            <div className="space-y-1 lg:space-y-2">
              <span className="block text-[8px] uppercase tracking-[0.2em] font-bold text-rose">Bespoke</span>
              <span className="block text-[9px] lg:text-[10px] uppercase tracking-widest font-bold">Community</span>
            </div>
          </div>
        </div>
      </div>

      {/* Footer / Scrolling Marquee Style Detail */}
        ...
        <div className="flex whitespace-nowrap overflow-hidden gap-8 lg:gap-12">
          {Array.from({ length: 10 }).map((_, i) => (
            <span key={i} className="text-[9px] lg:text-[10px] uppercase tracking-[0.5em] font-bold text-rose">
              Kloset Kifayah • Your Community Wardrobe • Conscious Choice •
            </span>
          ))}
        </div>
      </div>
